...
const translateShow = payload => translate(`${payload.series.title} (S${payload.episodes[0].seasonNumber.padStart(2, '0')}E${payload.episodes[0].episodeNumber.padStart(2, '0')})${payload.episodes.length > 1 ? '+' + (payload.episodes.length - 1) + 'others' : ''}`, 'Episode(s) added', 'Episode(s) added', null);
